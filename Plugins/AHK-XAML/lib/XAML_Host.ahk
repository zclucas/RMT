...
class XAMLHost {
    ...
    static Prewarm(exePath := "") {
        ...
        } else if (A_IsCompiled && FileExist(A_ScriptDir "\Plugins\AHK-XAML\lib\dep\" baseDllName)) {
    ...
    static CompileEngine(libDir, sharedExe, extraResources := [], embedDeps := false) {
        ...
        ; --- WpfAnimatedGif (GIF动画支持) ---
        gifRefs := ""
        gifDll := libDir "\dep\WpfAnimatedGif.dll"
        if (FileExist(gifDll)) {
            gifRefs := ' /reference:"' gifDll '"'
            if (embedDeps) {
                gifRefs .= ' /resource:"' gifDll '",WpfAnimatedGif.dll'
            }
        }
    ...
    static CopyRequiredDlls(libDir, targetDir) {
        ...
        ; WpfAnimatedGif (GIF动画支持)
        if FileExist(depDir "\WpfAnimatedGif.dll")
            try FileCopy(depDir "\WpfAnimatedGif.dll", targetDir "\WpfAnimatedGif.dll", 1)
    }
    ...
    _EnsureDaemon() {
        ...
        ; 检查守护进程是否存活，若已崩溃则重置以触发重启
        if (XAMLHost.daemonHwnd && !WinExist("ahk_id " XAMLHost.daemonHwnd)) {
            XAMLHost.daemonHwnd := 0
            XAMLHost._instances := Map()
        }
        if XAMLHost.daemonHwnd
            return targetExe
        ...
        if (A_IsCompiled && FileExist(A_ScriptDir "\" baseDllName)) {
            ...
            MsgBox("Error: The companion DLL '" baseDllName "' was not found.`n`nSearched:`n  - " A_ScriptDir "\" baseDllName "`n  - " A_ScriptDir "\Plugins\AHK-XAML\lib\dep\" baseDllName, "AHK-XAML", "Iconx")
            return ""
        } else if (!A_IsCompiled) {
            sourceCs := libDir "\dep\XAML_AHK_Bridge.cs"
    ...
    static Downloader(packageName, version, url, installDir, targetDlls, extractCommands) {
        ...
        try {
            ...
            MsgBox("Failed to restore " packageName " " version ":`n`n" e.Message, "Bootstrap Restore Error", "Iconx")
        }
